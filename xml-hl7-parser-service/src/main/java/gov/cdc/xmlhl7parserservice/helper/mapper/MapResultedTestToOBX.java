...
public class MapResultedTestToOBX {
    ...
    public MapResultedTestToOBX(MapToObservationValue mapToObservationValue, MapToCEType mapToCEType,
        MapCodedToStringValue mapCodedToStringValue) {
    ...
    void mapResultedTestToOBX(LabReportEvent.ResultedTest resultedTest, int obrSubCounter, int observationCounter,
        String OBXResult, int resultedTestCounter, int notesCounter,
        String referenceRangeFrom, String referenceRangeTo, String resultStatus, String messageType,
        String specimenCollectionDate,
        int obrCounter, EIElement eiElement, String dateTimeOfAnalysis, String interpretationFlag, int obxSubidCounter,
        ...
        for (MessageElement messageElement : resultedTest.getMessageElement()) {
            ...
            if (hl7Field.startsWith("OBX-")) {
                ...
                if (dataElement.contains("5.")) {
                    int counter = oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter)
                        .getOBSERVATION(observationCounter).getOBX().getObservationValue().length;
                    if (counter > 0) {
                        resultedTestCounter = resultedTestCounter + 1;
                    }
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter)
                        .getOBSERVATION(observationCounter).getOBX().getSetIDOBX()
                        .setValue(String.valueOf(resultedTestCounter + 1));
                }
                ...
                //TODO - Check if this value is being used
                    ...
                    String codedValue =
                        mapCodedToStringValue.mapCodedToStringValue(messageElement, OBXResult, parentLink);
                } else if (dataElement.contains("5.")) {
                    if (messageElement.getDataElement().getQuestionDataTypeNND().equals("SN_WITH_UNIT")) {
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter)
                            .getOBSERVATION(observationCounter).getOBX().getValueType().setValue("SN");
                        valueTypeChecker = 1;
                        ...
                        if (messageElement.getDataElement().getSnunitDataType().getCeLocalCodedValue() != null) {
                            localCodedValue =
                                messageElement.getDataElement().getSnunitDataType().getCeLocalCodedValue();
                        }

                        if (messageElement.getDataElement().getSnunitDataType()
                            .getCeLocalCodedValueDescription() != null) {
                            localCodedValueDescription =
                                messageElement.getDataElement().getSnunitDataType().getCeLocalCodedValueDescription();
                        }

                        if (messageElement.getDataElement().getSnunitDataType()
                            .getCeLocalCodedValueCodingSystem() != null) {
                            localCodedValueCodingSystem =
                                messageElement.getDataElement().getSnunitDataType().getCeLocalCodedValueCodingSystem();
                        }

                        CE currentObxUnits = oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter)
                            .getOBSERVATION(observationCounter).getOBX().getUnits();
                        ...
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter)
                            .getOBSERVATION(observationCounter).getOBX().getValueType()
                            .setValue(messageElement.getDataElement().getQuestionDataTypeNND());
                        valueTypeChecker = 1;
                    }

                    String observationValue = mapToObservationValue.mapToObservationValue(messageElement,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter)
                            .getOBSERVATION(observationCounter).getOBX().getObservationValue(0).toString());
                    ...
                    if (messageElement.getQuestionIdentifier()
                        .equals("LAB119") || messageElement.getQuestionIdentifier().equals("NBS373")) {
                        referenceRangeFrom = messageElement.getDataElement().getStDataType().getStringData();
                    }
                    if (messageElement.getQuestionIdentifier()
                        .equals("LAB120") || messageElement.getQuestionIdentifier().equals("NBS374")) {
                    ...
                    resultMethod.setNameOfCodingSystem(
        ...
        String obxValueType =
            oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter).getOBSERVATION(observationCounter)
                .getOBX().getValueType().getValue();
        ...
        if (obxValueType == null || obxValueType.equals("")) {
            ...
            int length =
                oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter).getSPECIMEN(observationCounter)
                    .getOBXReps();
        ...
        if (valueTypeChecker == 0) {
            int counter =
                oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter).getOBSERVATION(observationCounter)
                    .getOBX().getObservationValue().length;
            if (counter > 0) {
                resultedTestCounter = resultedTestCounter + 1;
            }
            Type obxValue = oruMessage.getPATIENT_RESULT()
            ...
            stDataType.setValue("\"\"");
            oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter).getOBSERVATION(observationCounter)
                .getOBX().getObservationValue(0).setData(stDataType);
            oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter).getOBSERVATION(observationCounter)
                .getOBX().getSetIDOBX().setValue(String.valueOf(resultedTestCounter + 1));
            oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrSubCounter).getOBSERVATION(observationCounter)
                .getOBX().getValueType().setValue("TX");
        }
    }
