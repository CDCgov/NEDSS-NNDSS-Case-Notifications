...
public class ConfigurationController {
  ...
  @Operation(
      ...
      requestBody =
          @io.swagger.v3.oas.annotations.parameters.RequestBody(
              ...
              content =
                  @Content(schema = @Schema(implementation = CaseNotificationConfigDto.class))))
  @PostMapping("/api/notification-config")
  public ResponseEntity<?> saveNotificationConfig(
      @RequestBody CaseNotificationConfigDto dto, HttpServletRequest request) {

    try {
      CaseNotificationConfig savedEntity = configurationService.saveConfig(dto);
      return ResponseEntity.status(HttpStatus.CREATED)
          .body(CaseNotificationConfigDto.fromEntity(savedEntity));
    } catch (Exception e) {
  ...
  @Operation(
      ...
      description =
          "If 'id' is not provided, updates the top config by name; otherwise updates by given id.",
      parameters = {
        @Parameter(
            in = ParameterIn.HEADER,
            name = "clientid",
