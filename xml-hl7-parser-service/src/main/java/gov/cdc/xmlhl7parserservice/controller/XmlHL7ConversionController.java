...
public class XmlHL7ConversionController {
  ...
  public XmlHL7ConversionController(
      HL7MessageBuilder hl7MessageBuilder, CNTraportqOutRepository cnTraportqOutRepository) {
  ...
  public ResponseEntity<String> convertXmlToHl7(
    ...
    try (InputStream is = getClass().getClassLoader().getResourceAsStream("sample_xml_dts1.xml")) {
      ...
      System.setProperty(
          "jakarta.xml.bind.context.factory", "org.glassfish.jaxb.runtime.v2.ContextFactory");
  ...
  public ResponseEntity<String> convertXmlToHl7ById(
      @PathVariable String recordId,
      @RequestParam(defaultValue = "true") boolean validationEnabled) {
    try {
      String messagePayload =
          cnTraportqOutRepository.findTopByRecordUid(Long.valueOf(recordId)).getMessagePayload();
      ...
      log.error(
          "Error during XML to HL7 conversion for recordId {}: {}", recordId, e.getMessage(), e);
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body("Failed to convert XML to HL7: " + e.getMessage());
    }
  }
