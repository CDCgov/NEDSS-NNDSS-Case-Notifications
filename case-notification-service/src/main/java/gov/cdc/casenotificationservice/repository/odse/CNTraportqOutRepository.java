...
public interface CNTraportqOutRepository extends JpaRepository<CNTransportqOut, Long> {
    @Query(value = "SELECT TOP 1 * FROM dbo.CN_transportq_out WHERE record_status_cd = :recordStatusCd",
        nativeQuery = true)
    CNTransportqOut findTopByRecordStatusCd(@Param("recordStatusCd") String recordStatusCd);

    @Query(
        value = "SELECT TOP 1 * FROM dbo.CN_transportq_out WHERE record_status_cd = :recordStatusCd AND cn_transportq_out_uid = :cnUid",
        nativeQuery = true)
    CNTransportqOut findTopByRecordStatusCdAndUid(@Param("recordStatusCd") String recordStatusCd,
        @Param("cnUid") Long cnUid);
    ...
    @Query(
        value = "SELECT TOP 1 * FROM dbo.CN_transportq_out WHERE message_payload NOT LIKE '%STD_MMG_V1.0%'   ORDER BY add_time DESC",
        nativeQuery = true)
    CNTransportqOut findTopNonStdForTesting();

    @Query(
        value = "SELECT TOP 1 * FROM dbo.CN_transportq_out WHERE message_payload LIKE '%STD_MMG_V1.0%'   ORDER BY add_time DESC",
        nativeQuery = true)
