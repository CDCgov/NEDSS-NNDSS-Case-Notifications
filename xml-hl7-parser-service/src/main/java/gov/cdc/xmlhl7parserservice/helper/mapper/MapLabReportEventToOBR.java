...
public class MapLabReportEventToOBR {
    ...
    public MapLabReportEventToOBR(MapToEIType mapToEIType, MapToCEType mapToCEType, MapToCWEType mapToCWEType,
        MapToTS18DataType mapToTS18DataType, MapResultedTestToOBX mapResultedTestToOBX, MapToDTM18 mapToDTM18) {
    ...
    public void mapLabReportEventToOBR(NBSNNDIntermediaryMessage nbsnndIntermediaryMessage,
        LabReportEvent labReportEvent, int obrCounter, int labObrCounter, String messageType, int i,
        EIElement eiElement, ParentLink parentLink, int obxSubidCounter, String cachedOBX3data, ORU_R01 oruMessage)
        throws HL7Exception {
        int labSubCounter = 0;
        ...
        for (MessageElement messageElement : nbsnndIntermediaryMessage.getMessageElement()) {
            ...
            if (hl7Field.startsWith("OBR-")) {
                ...
                if (dataElement.startsWith("1.")) {
                    ...
                    mapToEIType.mapToEIType(messageElement,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBR()
                            .getPlacerOrderNumber());
                } else if (dataElement.startsWith("3.")) {
                    EI fillerOrder =
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBR().getFillerOrderNumber();
                    ...
                    mapToCEType.mapToCEType(messageElement,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBR()
                    ...
                    mapToTS18DataType.mapToTS18DataType(
                    ...
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBR().getResultStatus()
                        .setValue(messageElement.getDataElement().getIdDataType().getIdCodedValue());
                } else if (dataElement.startsWith("31.")) {
                    mapToCEType.mapToCEType(messageElement,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBR()
                            .getReasonForStudy(reasonForStudyCounter));
                ...
                if (dataElement.startsWith("1.")) {
                    ...
                    mapToEIType.mapToEIType(messageElement,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getSPECIMEN(specimenCounter)
                            .getSPM().getSpecimenID().getPlacerAssignedIdentifier());
                } else if (dataElement.startsWith("2.2")) {
                    mapToEIType.mapToEIType(messageElement,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getSPECIMEN(specimenCounter)
                            .getSPM().getSpecimenID().getFillerAssignedIdentifier());
                } else if (dataElement.startsWith("4.")) {
                    mapToCWEType.mapToCWEType(messageElement,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getSPECIMEN(specimenCounter)
                            .getSPM().getSpecimenType());
                    isValidSPM = 1;
                } else if (dataElement.startsWith("8")) {
                    mapToCWEType.mapToCWEType(messageElement,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getSPECIMEN(specimenCounter)
                            .getSPM().getSpecimenSourceSite());
                    ...
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getSPECIMEN(specimenCounter)
                        .getSPM().getSpecimenCollectionAmount().getQuantity().setValue(quantity);
                } else if (dataElement.startsWith("12.1")) {
                } else if (dataElement.startsWith("12.2")) {
                    mapToCEType.mapToCEType(messageElement,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getSPECIMEN(specimenCounter)
                            .getSPM().getSpecimenCollectionAmount().getUnits());
                } else if (dataElement.startsWith("14")) {
                    String desc = messageElement.getDataElement().getStDataType().getStringData();
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getSPECIMEN(specimenCounter)
                        .getSPM().getSpecimenDescription(0).setValue(desc);
                } else if (dataElement.startsWith("17")) {
                    mapToTS18DataType.mapToTS18DataType(
                        messageElement.getDataElement().getTsDataType().getTime().toString(),
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getSPECIMEN(specimenCounter)
                            .getSPM().getSpecimenCollectionDateTime().getRangeStartDateTime());
            ...
            if (isValidSPM == 0) {
                ...
                oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getSPECIMEN(specimenCounter).getSPM()
                    .getSetIDSPM().setValue("1");
            }
        }

        if (eiElement.getEntityIdentifier() != null && !eiElement.getEntityIdentifier().isEmpty()) {
            EI fillerAssignedIdentifier =
                oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBR().getParentNumber()
        ...
        if (parentLink.getObservationValue() != null && !parentLink.getObservationValue().isEmpty()) {
            CE parentObservationIdentifier =
                oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBR().getParentResult()
            ...
            parentObservationIdentifier.getNameOfAlternateCodingSystem()
                .setValue(parentLink.getNameOfAlternateCodingSystem());
            parentObservationIdentifier.getAlternateIdentifier().setValue(parentLink.getAlternateIdentifier());

            oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBR().getParentResult()
                .getParentObservationSubIdentifier().setValue(parentLink.getObservationSubID());
            oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBR().getParentResult()
                .getParentObservationValueDescriptor().setValue(parentLink.getObservationValue());
        }
        ...
        for (int j = 0; j < resultedTestList.size(); j++) {
            ...
            for (int k = previousCounter; k < oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter)
                .getOBSERVATION(labSubCounter).getOBX().numFields(); k++) {
                if (oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labObrCounter)
                    .getOBX().getObservationIdentifier().getIdentifier().isEmpty()) {
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                        .getOBX().getObservationIdentifier().getIdentifier().setValue(identifier);
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                        .getOBX().getObservationIdentifier().getNameOfCodingSystem().setValue(description);
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                        .getOBX().getObservationIdentifier().getText().setValue(descriptionValue);
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                        .getOBX().getObservationResultStatus().setValue(resultStatus);
                }
                ...
                if (cachedOBX3data != null && !cachedOBX3data.isEmpty() && cachedOBX3data.contains(identifier + ":")) {
                    ...
                    if (start == 0) {
                        ...
                        int lookaheadLength =
                            Math.min(cachedOBX3data.length() - contTest, identifier.length() + 6); // safe bounds
                ...
                if (referenceRangeFrom.isEmpty() && !referenceRangeTo.isEmpty()) {
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                        .getOBX().getReferencesRange().setValue(referenceRangeFrom + "-" + referenceRangeTo);
                ...
                if (!specimenCollectionDate.isEmpty()) {
                    mapToDTM18.mapToDTM18(specimenCollectionDate,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                            .getOBX().getDateTimeOfTheObservation().getTime());
                }
                if (!dateTimeOfAnalysis.isEmpty()) {
                    mapToDTM18.mapToDTM18(dateTimeOfAnalysis,
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                            .getOBX().getDateTimeOfTheAnalysis().getTime());
                }
                if (!interpretationFlag.isEmpty()) {
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                        .getOBX().getAbnormalFlags(0).setValue(interpretationFlag);
                }
                if (resultMethod != null && !resultMethod.getText().isEmpty()) {
                    CE observationMethod =
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                            .getOBX().getObservationMethod(0);
                ...
                oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter).getOBX()
                    .getObservationSubID().setValue(Integer.toString(obxSubidCounter));
                String cweOBX =
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                        .getOBX().getValueType().toString();
                String ceOBX =
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                        .getOBX().getValueType().toString();
                CE observationIdentifier =
                    oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                        .getOBX().getObservationIdentifier();
                if ("CWE".equals(cweOBX) || "CE".equals(ceOBX)) {
                    ...
                    parentLinkOBX.setNameOfAlternateCodingSystem(
                        observationIdentifier.getNameOfAlternateCodingSystem().getValue());
                    parentLinkOBX.setAlternateText(observationIdentifier.getAlternateText().getValue());
                    parentLinkOBX.setObservationSubID(
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                            .getOBX().getObservationSubID().getValue());
                    // TODO - Check the toString() method
                    parentLinkOBX.setObservationValue(
                        oruMessage.getPATIENT_RESULT().getORDER_OBSERVATION(obrCounter).getOBSERVATION(labSubCounter)
                            .getOBX().getObservationValue(0).toString());
    ...
    void mapToSusceptibilityOBX(NBSNNDIntermediaryMessage nbsnndIntermediaryMessage,
        LabReportEvent.ResultedTest resultedTest, int obrCounter, int labSubCounter, int resultedTestCounter, int i,
        String messageType, ParentLink parentLink, EIElement eiElement, ORU_R01 oruMessage) throws HL7Exception {
        int labObrCounter;
